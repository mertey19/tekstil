...
import { blogPosts, getBlogPost, blogDate, readingMinutes } from "@/data/blog";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      ...(siteConfig.url ? { images: [{ url: new URL(post.image.src, siteConfig.url).toString(), alt: post.image.alt }] } : {}),
    },
  };
}
    ...
    <div className="container">
      ...
      <article className="blog-article">
        ...
        <figure className="blog-article-image">
          <ProductImage src={post.image.src} alt={post.image.alt} priority sizes="(max-width: 1100px) 100vw, 1080px" />
        ...
        <div className="blog-reading-layout">
          <nav className="blog-contents" aria-label="Yazı içindekiler">
            ...
            <Link className="text-link" href="/blog">Tüm yazılar <Icon size={16} /></Link>
          </nav>
          <div className="blog-prose">
            ...
            {post.sections.map((section) => (
              <section key={section.id} aria-labelledby={section.id}>
                ...
                {section.source && (
                  <p className="blog-source">Kaynak: <a href={section.source.url} target="_blank" rel="noopener noreferrer">{section.source.label}<span className="sr-only"> (yeni sekmede)</span></a></p>
                )}
              </section>
            ))}
          </div>
      ...
      <section className="blog-related" aria-labelledby="related-posts-title">
        <div className="section-heading">
          <div><span className="eyebrow">OKUMAYA DEVAM EDİN</span><h2 id="related-posts-title">Diğer rehberler</h2></div>
